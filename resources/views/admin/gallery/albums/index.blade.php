...
@section('content')
    ...
    <div class="stats-bar">
        <div class="stat-chip">
            ...
            Total Album: <strong>{{ $albums->total() }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-layer-group"></i>
            Halaman ini: <strong>{{ $albums->count() }}</strong>
        </div>
    </div>
                ...
                <table class="albums-table" id="albumsTable">
                    ...
                    <tbody>
                        @foreach($albums as $album)
                            <tr class="alb-row"
                                data-name="{{ strtolower($album->name) }}"
                                data-year="{{ $album->year }}">
                                <td style="color:#CBD5E1; font-size:0.8rem; font-weight:600;">
                                    {{ $loop->iteration + ($albums->currentPage() - 1) * $albums->perPage() }}
                                </td>
                                <td>
                                    @if($album->cover_image ?? false)
                                        <img src="{{ asset('storage/' . $album->cover_image) }}"
                                             class="alb-thumb" alt="{{ $album->name }}">
                                    @else
                                ...
                                <td style="max-width: 220px;">
                                    <div class="alb-name">{{ $album->name }}</div>
                                    <div class="alb-slug">{{ $album->slug }}</div>
                                </td>
                                <td>
                                    <span class="badge-year">{{ $album->year }}</span>
                                </td>
                                ...
                                <td>
                                    <span class="badge-items">
                                        <i class="fas fa-th-large"></i> {{ $album->items_count ?? 0 }} item
                                    </span>
                                </td>
                                <td>
                                    <div style="display:flex; align-items:center; justify-content:center; gap:6px; flex-wrap: nowrap;">
                                        ...
                                        <form action="{{ route('admin.albums.destroy', $album) }}"
                                            ...
                                            <button type="submit" class="act-btn delete" title="Hapus"
                                                    onclick="return confirm('Yakin ingin menghapus album \'{{ addslashes($album->name) }}\'?\nSemua item di dalamnya juga akan dihapus.')">
