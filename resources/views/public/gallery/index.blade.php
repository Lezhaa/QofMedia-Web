...
<section class="gal-hero" id="gal-hero">
    ...
    <div class="gal-hero-bg" id="heroBg">
        @php $bgPhotos = $highlightPhotos ?? collect(); @endphp
        @for($col = 0; $col < 5; $col++)
            <div class="gal-hero-bg-col">
                @for($row = 0; $row < 4; $row++)
                    @php $p = $bgPhotos->get($col * 4 + $row); @endphp
                    @if($p)
                        <img src="{{ asset('storage/' . $p->file_path) }}" alt="">
                    @else
    ...
    <div class="container" id="heroContent">
        ...
        <div class="gal-counter" id="galCounter">
            <div class="gal-counter-item">
                <span class="num" data-target="{{ $highlightPhotos->count() }}" data-suffix="+">{{ $highlightPhotos->count() }}+</span>
                <div class="lbl">Foto Terbaru</div>
            </div>
            <div class="gal-counter-item">
                <span class="num" data-target="{{ $years->count() }}">{{ $years->count() }}</span>
                <div class="lbl">Tahun</div>
            </div>
            <div class="gal-counter-item">
                <span class="num" data-target="{{ \App\Models\Album::count() }}">{{ \App\Models\Album::count() }}</span>
...
<section class="highlight-sec">
    <div class="container">
        ...
        <div class="bento-grid" id="bentoGrid">
            @foreach($highlightPhotos->take(8) as $photo)
                <div class="bento-item">
                    ...
                    <div class="bento-overlay">
                        <div class="bento-caption">
                            <div class="bc-title">{{ Str::limit($photo->caption, 40) ?: 'Foto Kegiatan' }}</div>
                            <div class="bc-sub">
                                <i class="bi bi-folder2" style="font-size:11px;"></i>
                                {{ $photo->album->name ?? 'Album' }}
                                @if($photo->album->year ?? false)
                                    &middot; {{ $photo->album->year }}
                                @endif
                            </div>
                        </div>
                    </div>
