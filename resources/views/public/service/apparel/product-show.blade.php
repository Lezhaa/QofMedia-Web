    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('service.apparel') }}">Qof Apparel</a></li>
...
<section class="detail-section">
    <div class="container">
        <a href="{{ route('service.apparel') }}" class="btn-back">
        ...
        <div class="row g-5">
            ...
            <div class="col-lg-5">
                <div class="product-gallery">
                    <div class="main-image-wrapper" id="mainImageWrapper">
                        ...
                        @endphp
                        @if($mainSrc)
                            <img id="mainImage" src="{{ $mainSrc }}" alt="{{ $product->name }}">
                        @else
            ...
            <div class="col-lg-7">
                <div class="product-info-card">
                    <div class="mb-2">
                        <span class="badge-category main">{{ $product->category->name ?? 'Produk' }}</span>
                    ...
                    <div class="product-price-large" id="productPrice">Rp {{ number_format($product->price, 0, ',', '.') }}</div>

                    @if($product->type === 'kaos' && $editions->count() > 0)
                        {{-- Series --}}
                        <div class="variant-section">
                            <span class="variant-label">Series: <span class="selected-value" id="selEditionLabel">Pilih</span></span>
                            <div class="variant-pills">
                                @foreach($editions as $edition)
                                    <button class="variant-pill edition-pill" data-edition="{{ $edition->id }}" onclick="selectEdition('{{ $edition->id }}', this)">{{ $edition->name }}</button>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="variant-pills" id="modelPills">
                                @foreach($models as $model)
                                    <button class="variant-pill model-pill"
                                            ...
                                            data-image="{{ $model->design_image ? asset('storage/'.$model->design_image) : '' }}"
                                            onclick="selectModel('{{ $model->id }}', this)"
                                            style="display:none;">{{ $model->name }}</button>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="variant-section">
                            ...
                            <div class="variant-pills" id="colorPills">
                                @foreach($colors as $color)
                                    <button class="variant-pill color-pill" data-color="{{ $color }}" onclick="selectColor('{{ $color }}', this)">
                                        <span class="color-dot" style="background:{{ strtolower($color) }};"></span>{{ ucfirst($color) }}
                                    </button>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="variant-section">
                            ...
                            <div class="variant-pills" id="sizePills">
                                @foreach($sizes as $size)
                                    <button class="variant-pill size-pill" data-size="{{ $size }}" onclick="selectSize('{{ $size }}', this)">{{ $size }}</button>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @else
                        <div class="stock-info-bar">
                            <span class="stock-tag {{ $product->stock > 0 ? 'ready' : 'out' }}">
                    ...
                    <p class="description-text">{{ $product->description ?: 'Tidak ada deskripsi.' }}</p>
                    ...
                    <div class="action-buttons">
                        ...
                        @endauth
                        <a href="{{ route('service.apparel') }}" class="btn-back-outline">
                ...
                <div class="row g-4">
                    @foreach($relatedProducts as $related)
                        <div class="col-md-3 col-6">
                            <a href="{{ route('apparel.product.show', $related->id) }}" class="text-decoration-none">
                                <div class="related-card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <p class="card-price mb-0">Rp {{ number_format($related->price, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                            </a>
                        </div>
            ...
            <form action="{{ route('order.store') }}" method="POST">
                ...
                <div class="modal-body px-4">
                    <div class="alert" style="background:#F0FDFA; border:1px solid #99F6E4; border-radius:12px; padding:12px 16px;">
                        ...
                        <div id="orderSummary" class="mt-1 small text-muted">{{ $product->name }}</div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Nama <span class="text-danger">*</span></label>
                            <input type="text" name="pemesan_name" class="form-control" value="{{ Auth::user()->name }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
