    ...
    <div class="container">
        <div class="tab-content" id="studioTabContent">
                    ...
                    @forelse($tools as $tool)
                        <div class="col-6 col-md-4 col-lg-3">
                            <div class="service-card h-100">
                                @if($tool->image)
                                    <img src="{{ asset('storage/' . $tool->image) }}" class="card-image" alt="{{ $tool->name }}" loading="lazy">
                                @else
                                ...
                                <span class="badge-stock {{ $tool->is_available && $tool->stock > 0 ? 'available' : 'unavailable' }}">
                                    {{ $tool->is_available && $tool->stock > 0 ? 'Tersedia' : 'Habis' }}
                                </span>

                                <p class="fw-bold mb-1" style="font-size:0.85rem;color:#0D1B2A;margin:4px 0 2px;">{{ $tool->name }}</p>
                                <span class="badge-category">{{ $tool->category }}</span>
                                <p class="text-muted mb-1" style="font-size:0.75rem;line-height:1.4;">{{ Str::limit($tool->description, 60) }}</p>

                                <div class="price-tag">
                                    Rp {{ number_format($tool->price_per_day, 0, ',', '.') }} <small>/ hari</small>
                                </div>
                                <p class="text-muted mb-2" style="font-size:0.72rem;">Stok: {{ $tool->stock }}</p>

                                @if($tool->is_available && $tool->stock > 0)
                                    <a href="{{ route('rental.booking.create', $tool) }}"
                                       class="btn-wa">
            ...
            <div class="tab-pane fade" id="studio-package" role="tabpanel">
                ...
                <div class="row g-4">
                    @forelse($studioPackages as $package)
                        <div class="col-6 col-md-4 col-lg-3">
                            <div class="service-card h-100 text-center">
                                <div class="service-icon-circle">
                                    <i class="bi bi-camera-reels-fill"></i>
                                </div>
                                <p class="fw-bold mb-1" style="font-size:0.85rem;color:#0D1B2A;">{{ $package->name }}</p>
                                <span class="badge-category">{{ $package->type ?? 'Studio' }}</span>
                                <p class="text-muted mb-1" style="font-size:0.75rem;line-height:1.4;">{{ $package->description }}</p>
                                <div class="price-tag mb-1">Rp {{ number_format($package->price, 0, ',', '.') }}</div>
                                <p class="text-muted mb-2" style="font-size:0.72rem;">Durasi: {{ $package->duration }}</p>

                                @php $facilities = is_string($package->facilities) ? json_decode($package->facilities, true) : $package->facilities; @endphp
                                @if(!empty($facilities) && is_array($facilities))
                                    <ul class="feature-list mb-3 text-start">
                                        @foreach($facilities as $facility)
                                            <li><i class="bi bi-check-circle-fill"></i> {{ $facility }}</li>
                                        @endforeach
                                    </ul>
                                @endif

                                <a href="https://wa.me/{{ $whatsappNumber ?? '6281246943349' }}?text={{ urlencode('Halo, saya tertarik paket ' . $package->name) }}"
            ...
            <div class="tab-pane fade" id="photo" role="tabpanel">
                ...
                <div class="row g-4">
                    @forelse($photoPackages as $package)
                        <div class="col-6 col-md-4 col-lg-3">
                            <div class="service-card h-100 text-center">
                                ...
                                <p class="fw-bold mb-1" style="font-size:0.85rem;color:#0D1B2A;">{{ $package->name }}</p>
                                <div class="price-tag mb-1">Rp {{ number_format($package->price, 0, ',', '.') }}</div>
                                <p class="text-muted mb-1" style="font-size:0.72rem;">{{ $package->duration }}</p>
                                <p class="text-muted mb-2" style="font-size:0.75rem;line-height:1.4;">{{ Str::limit($package->description, 60) }}</p>

                                @php $features = is_string($package->features) ? json_decode($package->features, true) : $package->features; @endphp
                                ...
                                <a href="https://wa.me/{{ $whatsappNumber ?? '6281246943349' }}?text={{ urlencode('Halo, saya tertarik ' . $package->name) }}"
                                   class="btn-wa" target="_blank">
...
<section class="cta-studio">
    <div class="container">
        <div class="cta-studio-card">
            ...
            <a href="https://wa.me/{{ $whatsappNumber ?? '6285156073776' }}" target="_blank" class="btn-cta-wa">
