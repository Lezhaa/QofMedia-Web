...
@section('title', 'Form Penyewaan — ' . $tool->name)
...
<div class="booking-hero">
    <div class="container">
        <p style="font-size:0.8rem; color:rgba(255,255,255,0.5); margin-bottom:4px;">
            <a href="{{ route('service.studio') }}" style="color:rgba(255,255,255,0.5); text-decoration:none;">Studio</a>
        ...
        <div class="tool-summary-card">
            @if($tool->image)
                <img src="{{ asset('storage/' . $tool->image) }}" class="tool-summary-img" alt="{{ $tool->name }}">
            @else
                <div class="tool-summary-placeholder"><i class="bi bi-tools"></i></div>
            @endif
            <div>
                <div class="tool-summary-name">{{ $tool->name }}</div>
                <div class="tool-summary-price">
                    <strong>Rp {{ number_format($tool->price_per_day, 0, ',', '.') }}</strong> / hari
                    &nbsp;·&nbsp; Stok: {{ $tool->stock }} unit
...
<div class="booking-section">
    <div class="container">
        ...
        <div class="form-wrapper">
            ...
            <form action="{{ route('rental.booking.store', $tool) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-body">
                    <div class="f-grid-2" style="margin-bottom: 18px;">
                        <div>
                            ...
                            <input type="text" class="f-input @error('pemesan_name') is-invalid @enderror"
                                   id="pemesan_name" name="pemesan_name"
                                   value="{{ old('pemesan_name', auth()->user()?->name) }}"
                                   placeholder="Nama lengkap Anda" required>
                            @error('pemesan_name')
                                <div class="f-error"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="text" class="f-input @error('pemesan_phone') is-invalid @enderror"
                                   id="pemesan_phone" name="pemesan_phone"
                                   value="{{ old('pemesan_phone', auth()->user()?->phone) }}"
                    ...
                    <div class="f-grid-2" style="margin-bottom: 18px;">
                        <div>
                            ...
                            <input type="date" class="f-input @error('tanggal_mulai') is-invalid @enderror"
                        ...
                        <div>
                            ...
                            <input type="date" class="f-input @error('tanggal_selesai') is-invalid @enderror"
                    ...
                    <div style="margin-bottom: 18px; max-width: 220px;">
                        ...
                        <input type="number" class="f-input @error('qty') is-invalid @enderror"
                               id="qty" name="qty"
                               value="{{ old('qty', 1) }}"
                               min="1" max="{{ $tool->stock }}"
                               onchange="updateTotal()" required>
                        <div class="f-hint"><i class="bi bi-info-circle"></i> Maks. {{ $tool->stock }} unit</div>
                        @error('qty')
                            <div class="f-error"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea class="f-textarea @error('catatan_user') is-invalid @enderror"
                                  id="catatan_user" name="catatan_user"
                                  rows="3"
                                  placeholder="Tambahkan catatan atau kebutuhan khusus...">{{ old('catatan_user') }}</textarea>
                        @error('catatan_user')
                            <div class="f-error"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-body" style="padding-top: 20px;">
                    ...
                    <div class="upload-zone @error('bukti_transfer') is-invalid @enderror" id="uploadZone">
                    ...
                    @error('bukti_transfer')
                        <div class="f-error" style="margin-top:8px;"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
                    @enderror
                ...
                <div class="form-body" style="padding-top: 20px;">
                    ...
                    <div class="jaminan-grid">
                        ...
                        @foreach($jaminanOptions as $value => $opt)
                            <div class="jaminan-option">
                                <input type="radio" name="jenis_jaminan" id="jaminan_{{ $value }}"
                                       value="{{ $value }}"
                                       {{ old('jenis_jaminan') === $value ? 'checked' : '' }}>
                                <label class="jaminan-label" for="jaminan_{{ $value }}">
                                    <div class="jaminan-icon"><i class="bi {{ $opt['icon'] }}"></i></div>
                                    <div style="flex:1;">
                                        <div class="jaminan-name">{{ $opt['name'] }}</div>
                                        <div class="jaminan-desc">{{ $opt['desc'] }}</div>
                                    </div>
                    ...
                    @error('jenis_jaminan')
                        <div class="f-error" style="margin-top:10px;"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
                    @enderror
                ...
                <div class="form-body" style="padding-top: 20px;">
                    ...
                    <div class="sk-box">
                        ...
                        <div class="sk-body" id="skBody">
                            ...
                            <div class="sk-item"><div class="sk-num">2</div><div>Alat harus dikembalikan sesuai <strong>tanggal selesai</strong> yang tercantum. Keterlambatan pengembalian dikenakan denda <strong>Rp {{ number_format($tool->price_per_day, 0, ',', '.') }}/hari</strong> (sesuai harga sewa).</div></div>
                    ...
                    {{-- Checkbox setuju --}}
                    <label class="agree-box @error('setuju_syarat') is-invalid @enderror" id="agreeBox" for="setuju_syarat">
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('service.studio') }}" class="btn-back">
...
@push('scripts')
<script>
    var pricePerDay = {{ (int) $tool->price_per_day }};
