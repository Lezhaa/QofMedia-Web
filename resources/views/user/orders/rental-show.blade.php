...
@section('title', 'Detail Penyewaan #' . $rentalBooking->id)
...
<section class="rental-hero">
    <div class="container">
        ...
        <p>Booking #{{ $rentalBooking->id }} &mdash; {{ $rentalBooking->created_at->format('d M Y') }}</p>
    </div>
</section>
...
<section class="rental-section">
    <div class="container">
        ...
        <a href="{{ route('user.orders', ['tab' => 'rental']) }}" class="btn-back">
                    ...
                    <div class="info-card-body">
                        <div class="alat-box">
                            @if($rentalBooking->tool?->image)
                                <img src="{{ asset('storage/' . $rentalBooking->tool->image) }}"
                                     alt="{{ $rentalBooking->tool->name }}"
                                     class="alat-thumb"
                            ...
                            @endif
                            <div class="alat-info">
                                <h5>{{ $rentalBooking->tool->name ?? 'Alat tidak ditemukan' }}</h5>
                                <p>{{ $rentalBooking->qty }} unit &bull; Rp {{ number_format($rentalBooking->tool->price_per_day ?? 0, 0, ',', '.') }} / hari</p>
                            </div>
                        </div>
                        ...
                        <div class="periode-box">
                            <div class="periode-date">
                                ...
                                <div class="date-val">{{ $rentalBooking->tanggal_mulai->format('d M Y') }}</div>
                            </div>
                            <i class="bi bi-arrow-right periode-arrow"></i>
                            <div class="periode-date">
                                <div class="date-label">Selesai Sewa</div>
                                <div class="date-val">{{ $rentalBooking->tanggal_selesai->format('d M Y') }}</div>
                            </div>
                            <span class="durasi-badge">
                                <i class="bi bi-calendar3"></i> {{ $rentalBooking->durasi }} Hari
                            </span>
                        </div>

                        <div class="info-row">
                            <span class="label"><i class="bi bi-box2 me-1"></i> Jumlah Unit</span>
                            <span class="value">{{ $rentalBooking->qty }} unit</span>
                        </div>
                        <div class="info-row">
                            <span class="label"><i class="bi bi-tag me-1"></i> Harga per Hari</span>
                            <span class="value">Rp {{ number_format($rentalBooking->tool->price_per_day ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="info-row">
                            <span class="label"><i class="bi bi-calendar-range me-1"></i> Durasi</span>
                            <span class="value">{{ $rentalBooking->durasi }} hari</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="total-box">
                    <div>
                        ...
                        <small style="opacity:0.6; font-size:0.75rem;">
                            {{ $rentalBooking->qty }} unit × {{ $rentalBooking->durasi }} hari × Rp {{ number_format($rentalBooking->tool->price_per_day ?? 0, 0, ',', '.') }}
                        </small>
                    </div>
                    <div class="total-amount">Rp {{ number_format($rentalBooking->total_harga, 0, ',', '.') }}</div>
                </div>
                ...
                <div class="info-card">
                    ...
                    <div class="info-card-body">
                        <div class="info-row">
                            ...
                            <span class="value">{{ $rentalBooking->pemesan_name }}</span>
                        </div>
                        <div class="info-row">
                            <span class="label"><i class="bi bi-telephone me-1"></i> Nomor HP</span>
                            <span class="value">{{ $rentalBooking->pemesan_phone }}</span>
                        </div>
                        <div class="info-row">
                            ...
                            <span class="value">
                                <span class="jaminan-pill">
                                    ...
                                    {{ $rentalBooking->jaminan_label }}
                                </span>
                            </span>
                        </div>
                        <div class="info-row">
                            <span class="label"><i class="bi bi-clock me-1"></i> Tanggal Pesan</span>
                            <span class="value">{{ $rentalBooking->created_at->format('d M Y, H:i') }} WIB</span>
                        </div>
                        ...
                        @if($rentalBooking->catatan_user)
                            <div class="mt-3">
                                ...
                                <div class="catatan-box">{{ $rentalBooking->catatan_user }}</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="info-card">
                    ...
                    <div class="info-card-body">
                        <div class="text-center mb-4">
                            <span class="status-badge {{ $rentalBooking->status }}" style="font-size:0.95rem; padding:10px 24px;">
                            ...
                            <ul class="timeline">
                                <li>
                                    ...
                                    <div class="tl-content">
                                        ...
                                        <div class="tl-desc">{{ $rentalBooking->created_at->format('d M Y') }}</div>
                                    </div>
                                </li>
                                <li>
                        ...
                        @else
                            <ul class="timeline">
                                @foreach($steps as $key => $step)
                                    ...
                                    @endphp
                                    <li>
                                        <div class="tl-dot {{ $isCurrent ? 'current' : ($isDone ? 'done' : '') }}">
                                            <i class="bi {{ $isDone ? 'bi-check-lg' : $step['icon'] }}"></i>
                                        </div>
                                        <div class="tl-content">
                                            <div class="tl-title" style="{{ $isCurrent ? 'color:#92400E;' : '' }}">{{ $step['label'] }}</div>
                    ...
                    <div class="info-card-body">
                        @if($rentalBooking->bukti_transfer)
                            <a href="{{ asset('storage/' . $rentalBooking->bukti_transfer) }}" target="_blank">
                                <img src="{{ asset('storage/' . $rentalBooking->bukti_transfer) }}"
                                     alt="Bukti Transfer"
                                     class="bukti-img">
                            </a>
                            <div class="text-center mt-2">
                                @if($rentalBooking->bukti_transfer_validated)
                            ...
                            <div class="text-center mt-2">
                                <a href="{{ asset('storage/' . $rentalBooking->bukti_transfer) }}"
                                   target="_blank"
                    ...
                    <div class="info-card-body">
                        ...
                        <div class="info-row" style="padding-top:14px; margin-top:4px; border-top:2px solid #E2E8F0; border-bottom:none;">
                            ...
                            <span class="value" style="font-size:1rem; color:#0E7A96; font-weight:800;">
                                Rp {{ number_format($rentalBooking->total_harga, 0, ',', '.') }}
                            </span>
                        </div>
                    </div>
                </div>
