    ...
    <div class="row g-3 mb-4 anim-1">
        ...
        <div class="col-6 col-lg-4">
            <div class="stat-card amber">
                ...
                <div>
                    <div class="stat-number">{{ $editions->pluck('year')->unique()->count() }}</div>
    ...
    <div class="dash-card anim-2">
        ...
        <div class="filter-bar">
            ...
            <select class="filter-select" id="filterYear">
                ...
                @foreach($editions->pluck('year')->unique()->sortDesc() as $year)
            ...
            <table class="dash-table" id="editionsTable">
                ...
                <tbody>
                    @forelse($editions as $edition)
                        <tr data-name="{{ strtolower($edition->name) }}"
                            data-product="{{ $edition->product->name ?? '' }}"
                            ...
                            <td>
                                <span class="product-pill">
                                    ...
                                    {{ $edition->product->name ?? 'N/A' }}
                                </span>
                            </td>
                            <td>
                                <div style="font-weight: 700; font-size: 0.88rem;">{{ $edition->name }}</div>
                                <div class="td-muted">ID #{{ $edition->id }}</div>
                            </td>
                            <td>
                            ...
                            <td style="text-align:center; white-space:nowrap;">
                                ...
                                <form action="{{ route('apparel.editions.destroy', $edition) }}"
                                      method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn-xs-act danger" title="Hapus"
                                            onclick="return confirm('Hapus edisi \'{{ $edition->name }}\'?')">
        ...
        {{-- No results row (hidden, shown by JS filter) --}}
        <div id="noResults"
