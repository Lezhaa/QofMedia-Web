...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('apparel.products.create') }}"
...
@section('content')
    ...
    <div class="stats-bar">
        <div class="stat-chip">
            ...
            Total Produk: <strong>{{ isset($products) ? $products->total() : 0 }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-box-open"></i>
            Halaman ini: <strong>{{ isset($products) ? $products->count() : 0 }}</strong>
        </div>
    </div>
    ...
    <div class="filter-bar">
        ...
        <select id="catFilter" onchange="filterRows()">
            ...
            @foreach(($products ?? collect())->pluck('category.name')->filter()->unique()->sort() as $cat)
                <option value="{{ strtolower($cat) }}">{{ $cat }}</option>
            @endforeach
        </select>
        <select id="typeFilter" onchange="filterRows()">
            <option value="">Semua Tipe</option>
            @foreach(($products ?? collect())->pluck('type')->filter()->unique()->sort() as $type)
                    ...
                    <tbody>
                        @foreach($products as $product)
                            <tr class="prod-row"
                                ...
                                <td>
                                    <span class="badge-cat">{{ $product->category->name ?? 'N/A' }}</span>
                                </td>
                                <td>
                                    <span class="badge-type {{ strtolower($product->type) == 'kaos' ? 'kaos' : '' }}">
                                ...
                                <td>
                                    <div style="display:flex; align-items:center; justify-content:center; gap:6px;">
                                        <a href="{{ route('apparel.products.edit', $product) }}"
                                           class="act-btn edit" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('apparel.products.destroy', $product) }}"
