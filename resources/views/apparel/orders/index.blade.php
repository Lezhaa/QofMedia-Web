...
@section('content')
    ...
    <div class="stats-bar">
        ...
        <div class="stat-chip">
            ...
            Menunggu: <strong style="color:#D97706;">{{ $orders->getCollection()->where('status','menunggu')->count() ?? 0 }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-check-circle" style="color:#059669;"></i>
            Disetujui: <strong style="color:#059669;">{{ $orders->getCollection()->where('status','disetujui')->count() ?? 0 }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-times-circle" style="color:#DC2626;"></i>
            Ditolak: <strong style="color:#DC2626;">{{ $orders->getCollection()->where('status','ditolak')->count() ?? 0 }}</strong>
        </div>
    </div>
    ...
    <div class="filter-bar">
        ...
        <a href="{{ route('apparel.orders.index', ['status' => 'menunggu']) }}"
           class="status-tab menunggu {{ request('status') == 'menunggu' ? 'active' : '' }}">
            <span class="tab-dot"></span> Menunggu
        </a>
        <a href="{{ route('apparel.orders.index', ['status' => 'disetujui']) }}"
           class="status-tab disetujui {{ request('status') == 'disetujui' ? 'active' : '' }}">
            <span class="tab-dot"></span> Disetujui
        </a>
        <a href="{{ route('apparel.orders.index', ['status' => 'ditolak']) }}"
           class="status-tab ditolak {{ request('status') == 'ditolak' ? 'active' : '' }}">
                ...
                <table class="orders-table" id="ordersTable">
                    ...
                    <tbody>
                        @foreach($orders as $order)
                            ...
                            @endphp
                            <tr class="ord-row"
                                data-name="{{ strtolower($order->pemesan_name . ' ' . ($order->product->name ?? '')) }}">
                                <td>
                                    <span class="order-id">#{{ $order->id }}</span>
                                </td>
                                <td>
                                    <div class="ord-name">{{ $order->pemesan_name }}</div>
                                    <div class="ord-phone">
                                        <i class="fas fa-phone"></i> {{ $order->pemesan_phone }}
                                    </div>
                                </td>
                                <td>
                                    <div class="ord-product">{{ $order->product->name ?? 'N/A' }}</div>
                                </td>
                                <td style="text-align:center;">
                                    <span class="badge-qty">{{ $order->qty }}</span>
                                </td>
                                <td>
                                    <span class="ord-total">Rp {{ number_format($total, 0, ',', '.') }}</span>
                                </td>
                                <td>
