...
@section('title', 'Kelola Model/Motif Kaos')
    ...
    <div class="row g-3 mb-4 anim-1">
        ...
        <div class="col-6 col-lg-3">
            <div class="stat-card green">
                ...
                <div>
                    <div class="stat-number">{{ $models->whereNotNull('design_image')->count() }}</div>
        ...
        <div class="col-6 col-lg-3">
            <div class="stat-card amber">
                ...
                <div>
                    <div class="stat-number">{{ $editions->count() }}</div>
        ...
        <div class="col-6 col-lg-3">
            <div class="stat-card red">
                ...
                <div>
                    <div class="stat-number">{{ $models->whereNull('design_image')->count() }}</div>
        ...
        <div class="dash-card-header">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                <span class="count-badge">
                    <i class="fas fa-list"></i> {{ $models->count() }} model
                </span>
                ...
                <table class="dash-table" id="modelsTable">
                    ...
                    <tbody>
                        @forelse($models as $model)
                            <tr data-name="{{ strtolower($model->name) }}"
                                data-edition="{{ $model->edition->name ?? '' }}"
                                data-hasimage="{{ $model->design_image ? '1' : '0' }}">
                                <td><span class="row-num">{{ $loop->iteration }}</span></td>
                                <td>
                                    @if($model->design_image)
                                        <img src="{{ asset('storage/'.$model->design_image) }}"
                                             class="design-thumb preview-trigger"
                                             data-src="{{ asset('storage/'.$model->design_image) }}"
                                ...
                                <td>
                                    <div style="font-weight:700; font-size:0.88rem;">{{ $model->name }}</div>
                                    <div class="td-muted">ID #{{ $model->id }}</div>
                                </td>
                                <td>
                                    <span class="edition-pill">
                                        <i class="fas fa-layer-group" style="font-size:0.72rem;"></i>
                                        {{ $model->edition->name ?? 'N/A' }}
                                    </span>
                                </td>
                                <td style="text-align:center; white-space:nowrap;">
                                    ...
                                    <form action="{{ route('apparel.models.destroy', $model) }}"
                                          method="POST" class="d-inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-xs-act danger" title="Hapus"
                                                onclick="return confirm('Hapus model \'{{ $model->name }}\'?')">
