{{-- ============================================================
     ...
     Diletakkan di resources/views/partials/navbar-notifications.blade.php
     ============================================================ --}}
...
<li class="nav-item dropdown" id="notif-nav-item">
    ...
    <div class="dropdown-menu dropdown-menu-end p-0 shadow"
        ...
        <div class="border-top text-center py-2" style="font-size:12px;">
            ...
            @else
                <a href="{{ route('information.index') }}" class="text-decoration-none">
...
(function () {
    ...
    function fetchCount() {
        fetch('{{ route("notifications.unread-count") }}', { credentials: 'same-origin' })
            .then(r => r.json())
            .then(d => {
                const b = badge();
                if (d.count > 0) {
    ...
    function fetchList() {
        ...
        fetch('{{ route("notifications.index") }}', { credentials: 'same-origin' })
            .then(r => r.json())
            .then(items => {
                if (!items.length) {
                    list().innerHTML = `
