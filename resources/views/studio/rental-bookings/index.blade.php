...
@section('title', 'Pemesanan Sewa Alat')
...
@section('content')
    ...
    <div class="stats-bar">
        <div class="stat-chip">
            ...
            <div class="stat-chip-info"><strong>{{ $stats['total'] }}</strong><span>Total Pemesanan</span></div>
        </div>
        <div class="stat-chip">
            <div class="stat-chip-icon amber"><i class="fas fa-clock"></i></div>
            <div class="stat-chip-info"><strong>{{ $stats['menunggu'] }}</strong><span>Menunggu</span></div>
        </div>
        <div class="stat-chip">
            <div class="stat-chip-icon blue"><i class="fas fa-tools"></i></div>
            <div class="stat-chip-info"><strong>{{ $stats['aktif'] }}</strong><span>Sedang Berjalan</span></div>
        </div>
        <div class="stat-chip">
            <div class="stat-chip-icon green"><i class="fas fa-check-double"></i></div>
            <div class="stat-chip-info"><strong>{{ $stats['selesai'] }}</strong><span>Selesai</span></div>
        </div>
    </div>

    {{-- Filter --}}
    <form method="GET" action="{{ route('studio.rental-bookings.index') }}">
        <div class="filter-bar">
            <i class="fas fa-search" style="color:#CBD5E1; font-size:0.9rem; flex-shrink:0;"></i>
            <input type="text" name="search" placeholder="Cari nama pemesan atau alat..." value="{{ request('search') }}">
            <select name="status">
                <option value="">Semua Status</option>
                @foreach(['menunggu'=>'Menunggu','disetujui'=>'Disetujui','aktif'=>'Sedang Berjalan','selesai'=>'Selesai','ditolak'=>'Ditolak'] as $val => $label)
                    <option value="{{ $val }}" {{ request('status') == $val ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
            <button type="submit" class="filter-btn"><i class="fas fa-filter"></i> Filter</button>
            @if(request('search') || request('status'))
                <a href="{{ route('studio.rental-bookings.index') }}" class="reset-btn">
    ...
    <div class="bookings-card">
        ...
        @else
                    ...
                    <tbody>
                        @foreach($bookings as $i => $booking)
                            <tr>
                                ...
                                <td>
                                    <div class="pemesan-name">{{ $booking->pemesan_name }}</div>
                                    <div class="pemesan-phone">{{ $booking->pemesan_phone }}</div>
                                </td>
                                <td>
                                    <div class="tool-name">{{ $booking->tool->name ?? '—' }}</div>
                                    <div class="tool-cat">{{ $booking->tool->category ?? '' }}</div>
                                </td>
                                <td>
                                    ...
                                    <div class="date-range durasi">{{ $booking->durasi }} hari</div>
                                </td>
                                <td style="font-size:0.85rem; font-weight:600;">{{ $booking->qty }} unit</td>
                                <td>
                                ...
                                <td>
                                    <div style="display:flex; align-items:center; justify-content:center; gap:6px;">
                                        <a href="{{ route('studio.rental-bookings.show', $booking) }}"
                                           class="act-btn view" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form action="{{ route('studio.rental-bookings.destroy', $booking) }}"
                                              method="POST" style="margin:0;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="act-btn delete" title="Hapus"
                                                    onclick="return confirm('Yakin hapus pemesanan dari {{ addslashes($booking->pemesan_name) }}?')">
            ...
            @if($bookings->hasPages())
                <div class="card-foot">{{ $bookings->links() }}</div>
            @endif
        @endif
    </div>
