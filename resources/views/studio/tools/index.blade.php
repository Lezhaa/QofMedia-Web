...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('studio.tools.create') }}" class="btn btn-primary" style="background: #0E7A96; border-color: #0E7A96; border-radius: 50px; padding: 10px 22px; font-weight: 600; font-size: 0.88rem;">
...
@section('content')
    ...
    <div class="stats-bar">
        <div class="stat-chip">
            ...
            Total Alat: <strong>{{ $tools->total() }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-check-circle" style="color: #10B981;"></i>
            Tersedia: <strong>{{ $tools->where('is_available', true)->count() }}</strong>
        </div>
        <div class="stat-chip">
            <i class="fas fa-times-circle" style="color: #EF4444;"></i>
            Tidak Tersedia: <strong>{{ $tools->where('is_available', false)->count() }}</strong>
        </div>
    </div>
    ...
    @if($tools->isEmpty())
        <div class="empty-state">
            ...
            <a href="{{ route('studio.tools.create') }}" class="btn btn-primary" style="background: #0E7A96; border-color: #0E7A96; border-radius: 50px; padding: 10px 24px; font-weight: 600;">
        ...
        <div class="row g-3" id="cardGrid">
            @foreach($tools as $tool)
                <div class="col-6 col-md-4 col-lg-3 card-item"
                     ...
                     data-status="{{ $tool->is_available && $tool->stock > 0 ? 'tersedia' : 'habis' }}">
                    <div class="tool-card">
                        ...
                        <div class="tool-card-body">
                            ...
                            <div class="tool-stock">
                                <i class="fas fa-layer-group me-1"></i> Stok: {{ $tool->stock }} unit
                            </div>
                            ...
                            <div class="tool-actions">
                                <a href="{{ route('studio.tools.edit', $tool) }}" class="btn-edit">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('studio.tools.destroy', $tool) }}" method="POST" style="flex: 1;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-delete"
                                            onclick="return confirm('Yakin ingin menghapus \'{{ $tool->name }}\'?')">
